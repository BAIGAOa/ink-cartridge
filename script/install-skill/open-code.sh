...
# Install repo skills into OpenCode (project-level .opencode/skills/).
#
# Copies every skill directory from skills/ (excluding README entries) into
# .opencode/skills/, where OpenCode discovers them as project skills.
# Re-runs are safe: the destination is rebuilt from scratch each time.
#
# Usage: ./script/install-skill/open-code.sh
