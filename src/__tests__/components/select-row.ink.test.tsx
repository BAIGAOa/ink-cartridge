...
import { SelectRow } from '../../components/select-row/SelectRow.js';
...
describe('滚动（items > limit）', () => {
  ...
  it('高亮向右移动超出窗口时自动滚动，新项出现旧项消失', async () => {
  ...
  it('高亮向左移动超出窗口时自动回滚，旧项重新出现', async () => {
    const { lastFrameClean, stdin } = renderSelectRow({
      focusId: 'test',
      items: longItems,
    });
...
describe('焦点系统', () => {
  it('两个 SelectRow 中，第一个注册的自动获得焦点', async () => {
  ...
  it('两个 SelectRow 各自的选中状态独立', async () => {
    const { stdin, onSelectA, onSelectB, kbRef } = renderDualSelectRow({
      items: threeItems,
    });
...
describe('组件卸载清理', () => {
  ...
  it('返回后重新进入，SelectRow 重新初始化并正常工作', async () => {
    const { stdin, onSelect, lastFrameClean } =
      renderNavigableSelectRow(threeItems);
...
describe('非聚焦渲染（isFocused=false）', () => {
  it('未获焦的 SelectRow 不显示 ● 指示器', async () => {
