...
When a key is marked transparent, the layer's own bindings for that key are skipped — the key continues to the next layer below, or to the page below in the screen stack. Penetration means "pass through", not "block".
...
| `focusId` | `string \| { group: string; focusId: string }` | — | Scope to a specific focus target on the current layer. |
...
Adds `KeyRule` entries to the layer's `penetrationKeys` array (or the focus target's). During key matching (layer broadcast and screen-stack stages), penetration keys are checked first:
...
- **[`allowModal`](./allowModal.md)** — conceptually similar (let keys through a barrier), but `penetration` works on layers and page layers while `allowModal` works on modal layers
