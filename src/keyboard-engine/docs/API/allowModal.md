...
By default, when a modal layer is active, the modal processor consumes every key event — even keys that have no binding on the modal layer. This prevents any key from reaching lower pipeline stages. `allowModal` creates an exception: keys on the allow list are released to continue through the pipeline (to global keys, layers, and page screens).
...
|--------|------|---------|-------------|
| `elementId` | `string` | — | Scope to a specific element on the modal layer. |
| `focusId` | `string \| { group: string; focusId: string }` | — | Scope to a specific focus target on the modal layer. |
...
Adds `KeyRule` entries to the modal element's `allowedKeys` array (or the named focus target's). During pipeline stage 0 (modal processor), `allowedKeys` is checked before any other processing:
...
- **[`penetration`](./penetration.md)** — conceptually similar (let keys through a barrier), but operates at different pipeline stages: `penetration` in layer/page matching, `allowModal` at stage 0 (modal)
