import { KeyRuleContainer, finalizeBoundKeyboard, pushKeyEntries } from "../providers/helpers.js";
import { AllowModalOptions, BoundKeyboardOptions, BoundKeyEntry, KeyHandler, ModalMissCallback, ModalMissOptions, PenetrationOptions, ScreenKeyboardLayer, SequenceBinding, SequenceOptions, StopOptions } from "../types.js";
import EngineState from "./EngineState.js";
import LayerManager from "./LayerManager.js";

export default class BindingService<TComponent = unknown> {

    constructor(private state: EngineState<TComponent>, private layers: LayerManager<TComponent>) {
    ...
    boundKeyboard(
        ...
        if (options?.focusId) {
            ...
            const target = this.layers.getOrCreateFocusTarget(layer, fid);
    ...
    stop(keys: string[], options?: StopOptions): () => void {
        ...
        if (options?.stopAction) {
            const map = options.focusId
                ? this.layers.getOrCreateFocusTarget(layer, options.focusId).actionKeysMap
                : layer.actionKeysMap;
            const merged: string[] = [];
