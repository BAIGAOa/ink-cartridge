...
/**
 * Developer debugging overlay for the ink-router-kit screen system.
 *
 ...
 * (green = active, gray = inactive), screen count, and the panel's
 * vertical offset.
 *
 ...
 *
 * Must be opened via {@link openDevTool} (which calls `openOverlay`).
 * The panel registers itself via `registerComponent` so it participates
 * in the overlay keyboard layer automatically.
 *
...
export function DevScreen({top: initialTop, left}: DevProps){
  const {currentPath, displayedOverlays, activeOverlayIds} = useScreenSystem()
  ...
  return(
    ...
    >
      ...
      <Box flexDirection="column">
        <Text dimColor>
          Screens: {currentPath.length}  |  ↑↓ Move
        </Text>
        <Text dimColor>
