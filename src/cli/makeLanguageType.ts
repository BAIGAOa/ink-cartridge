...
export function generateRuntimeContent(packageName: string): string {
  return (
    ...
    `// Do not edit manually.\n` +
    `\n` +
    `import { useI18n as rawUseI18n } from '${escapeSingleQuote(packageName)}';\n` +
    `import type { TranslationKey, TranslationParams } from './i18n-types.js';\n` +
    `\n` +
    `// Module-level ref set by any component that calls useI18n().\n` +
    `// The standalone t() delegates to it so no manual context threading\n` +
    `// is needed.\n` +
    ...
    ` * The ref is updated automatically when any component calls \`useI18n()\`.\n` +
    ` *\n` +
    ...
    `  if (!_t) {\n` +
    `    throw new Error(\n` +
    `      '[ink-cartridge] typed t() called before <LanguageProvider> is mounted. ' +\n` +
    `      'Ensure a component has called useI18n() first.',\n` +
    `    );\n` +
    `  }\n` +
    ...
    ` * with a type-safe \`t\` function and updates the module-level ref.\n` +
    ` */\n` +
...
function generate(sourceDir: string, outputDir: string, options: MakeLanguageTypeOptions): void {
  ...
  process.stderr.write(
    `[ink-cartridge] Generated ${commonKeys.length} shared keys → ${path.resolve(outputDir)}\n`,
  );
}
