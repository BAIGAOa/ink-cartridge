...
```tsx
function KeyboardProvider({ children, modes, defaultMode, processors, valueSchema, autoTab, mouse, mouseOptions }: KeyboardProviderProps): JSX.Element
```
...
- On every render, calls `engine.sync()` to push screen-path and layer/modal layer state into the engine.
- Runs each key event through the 9-stage pipeline (modal → composition → global sequences → global keys → layer broadcast → composition → global sequences → global keys → screen stack).
- Cleans up layers for screens, layers, and modal layers that leave the tree via post-render effects.
